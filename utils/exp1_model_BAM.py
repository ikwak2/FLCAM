...
class GLU2(nn.Module):
    ...
    def forward(self, x): #x size = [batch, chan, freq, frame]
        lin = self.linear(x) #x size = [batch, freq, frame, chan]
        sig = self.sigmoid(x)
        res = lin * sig
        return res
...
class ContextGating2(nn.Module):
    def __init__(self, in_dim):
        super(ContextGating2, self).__init__()
        self.sigmoid = nn.Sigmoid()
        self.sigmoid = nn.Sigmoid()
        self.linear = nn.Linear(in_dim, in_dim)
...
class CNN(nn.Module):
    def __init__(self,
                 ...
                 pooling=[(1, 4), (1, 4), (1, 4)],
                 normalization="batch",
                 pooling_layers=[1, 1, 1, 1, 1, 1, 1],
                 atte_layers=[1, 1, 1, 1, 1, 1, 1],
                 timedim=[626, 313, 156, 156, 156, 156, 156], 
                 temperature=31,
                 pool_dim='freq'):
