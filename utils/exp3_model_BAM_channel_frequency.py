...
#         self.gate_activation = gate_activation
        ...
        self.gate_f.add_module( 'flatten', Flatten() )
        ...
        for i in range( len(frequency_channels) - 2 ):
            self.gate_f.add_module( 'gate_f_fc_%d'%i, nn.Linear(frequency_channels[i], frequency_channels[i+1]) )
            self.gate_f.add_module( 'gate_f_bn_%d'%(i+1), nn.BatchNorm1d(frequency_channels[i+1]) )
            self.gate_f.add_module( 'gate_f_relu_%d'%(i+1), nn.ReLU() )
        self.gate_f.add_module( 'gate_f_fc_final', nn.Linear(frequency_channels[-2], frequency_channels[-1]) )
    def forward(self, in_tensor):
        x = in_tensor.permute(0,3,1,2)
        avg_pool = F.avg_pool2d( x, (x.size(2), x.size(3)), stride=(x.size(2), x.size(3)) )
        return self.gate_f( avg_pool ).unsqueeze(1).unsqueeze(2).expand_as(in_tensor) 
...
class BAM(nn.Module):
    def __init__(self, gate_channel, frequency_channel):
        ...
        self.frequency_att = FrequencyGate(frequency_channel)
    def forward(self,in_tensor):
        att =1 + F.sigmoid( self.frequency_att(in_tensor) *  self.channel_att(in_tensor))  
